...
public class compatible {
     public static void compatible(Ladka[] b,Ladki[] g,int i,int k) throws IOException
    {    
        ...
        int h,l,p;
         for(h=0;h<i;h++){//loop for ladke
            for(l=0;l<i;l++){//loop for ladki 
                if(b[h].pehlapyar ==(g[l].naam)){
                    p=abs(b[h].aaukat- g[l].exp)+abs(b[h].buddhi-g[l].buddhi)+abs(b[h].minimum_sundar-g[l].attr);
                    com[h][0]=p;
                    com[h][1]=h;
                } else {
                }
         ...
         System.out.println("-----------compatible_jodis-------------------------");
         for(h=0;h<k;h++){
             for(l=0;l<i;l++){
